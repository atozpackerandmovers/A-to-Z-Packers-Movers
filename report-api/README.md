# Secure Daily Operations + Master AI Report API

This staging package also exports `masterAiReport`, a Firebase-Auth-protected read-only endpoint for the Manoj Kumar Swain Master AI Business Director staging UI. It verifies the Firebase ID token and requires `masterAiAdmin: true` or a UID in server-only `MASTER_AI_ADMIN_UIDS`; no bearer secret is embedded in browser source.
